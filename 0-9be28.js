...
var aside = __webpack_require__(38);
var fx = __webpack_require__(39);
var task = __webpack_require__(44);
// Storage management
...
var sortTasks = function (tasks) {
    return (_a = []).concat.apply(_a, splitTasks(tasks));
    var _a;
};
var raf = function (fn) { return requestAnimationFrame(function () { return requestAnimationFrame(fn); }); };
var prepend = function (x) { return function (xs) { return [x].concat(xs); }; };
var through = function (fn) { return function (x) {
    fn(x);
    return x;
}; };
var wrap = function (obj) { return ({
    map: function (fn) { return wrap(fn(obj)); },
    unwrap: function () { return obj; },
}); };
var update = function (model, address) {
    var args = [];
    ...
    switch (address) {
        case Action.Update:
            var id = args[0], taskAddress_1 = args[1], taskArgs_1 = args.slice(2);
            return wrap(model)
                .map(duckweed.scoped.transform.bind(null, ["tasks", id], function (taskModel) { return task.update.apply(task, [taskModel, taskAddress_1].concat(taskArgs_1)); }))
                .map(duckweed.scoped.transform.bind(null, ["tasks"], function (tasks) { return sortTasks(tasks).filter(notEmpty); }))
                .map(through(function (m) { return store(m.tasks); }))
                .unwrap();
        case Action.Add:
            return duckweed.scoped.transform(["tasks"], prepend(task.init({ editing: true })), model);
        case Action.ClearDone:
            return duckweed.scoped.transform(["tasks"], filterDone.bind(null, false), model);
        case Action.SetHeight:
            var vnode = args[1] ? args[1] : args[0];
    ...
    return (duckweed.html("div", { class: elements.wrapper, style: fx.dropInTwistOut() },
        duckweed.html("main", { class: elements.main },
            ...
            duckweed.html("div", { class: css.tasks, "hook-insert": act(Action.SetHeight), "hook-postpatch": act(Action.SetHeight) }, model.tasks.map(function (item, index) {
                return duckweed.html(task.view, { model: item, act: act.bind(null, Action.Update, index), classes: [css.task], styles: {
                        delayed: {
                            opacity: 1,
                        },
                        opacity: 0,
...
var update = function (model, address) {
    ...
    switch (address) {
        case Action.Toggle:
            var checked = args[0];
            return __assign({}, model, { done: checked, editing: checked ? false : model.editing });
        case Action.ToggleEditing:
            return __assign({}, model, { editing: !model.editing });
        case Action.Update:
            var text = args[0];
            return __assign({}, model, { text: text });
        case Action.Focus:
            var vnode = args[0];
            vnode.elm.focus();
            return model;
...
// Utility functions
var matchRoute = function (rm, path) {
    return route.match("/duckweed-tasks", rm.re, path);
};
var getMatchingRoute = function (routes, path) {
    if (!routes.length) {
        return undefined;
    }
    var current = routes[0], remaining = routes.slice(1);
...
var update = function (model, address) {
    ...
    switch (address) {
        case Action.ModuleAction:
            var moduleUpdate_1 = args[0], moduleaddress_1 = args[1], moduleMsg_1 = args.slice(2);
            if (typeof moduleUpdate_1 === "undefined") {
                // The module does not define any actions, no point in doing anything
                return model;
            }
            return duckweed.scoped.transform(["model"], function (moduleModel) { return moduleUpdate_1.apply(void 0, [moduleModel, moduleaddress_1].concat(moduleMsg_1)); }, model);
        case Action.SwitchRoute:
            var pathname = args[0].pathname;
            var mod = getMatchingRoute(routeTable, pathname).mod;
            return __assign({}, model, { model: mod.init ? mod.init() : undefined, path: pathname });
        case Action.NavbarAction:
            var path_1 = args[0];
            setTimeout(function () { return route.go(path_1); });
            return model;
        default:
            return model;
    }
...
var view = function (_a) {
    ...
    return (function (_a) {
        ...
        return duckweed.html("div", { route: act(Action.SwitchRoute) },
            duckweed.html(navbar.view, { act: act.bind(null, Action.NavbarAction), links: model.links }),
            duckweed.html(mod.view, { model: model.model, act: act.bind(null, Action.ModuleAction, mod.update), key: model.path }));
    })(getMatchingRoute(routeTable, model.path));
};
exports.view = view;
...
var view = function (_a) {
    ...
    return (duckweed.html("nav", { class: css.nav }, links.map(function (_a) {
        var path = _a[0], label = _a[1];
        return duckweed.html("a", { class: css.link, "on-click": act(path) }, label);
    })));
};
exports.view = view;
...
//# sourceMappingURL=0-9be28.js.map